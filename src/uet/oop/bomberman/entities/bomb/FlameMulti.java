...
import uet.oop.bomberman.BombermanMultiPlayer;
...
public class FlameMulti extends Entity implements CollisionTypeCheck {
    private int left;
    private int right;
    private int top;
    private int down;
    ...
    public FlameMulti(int x, int y, Image image, int direction){
        super(x, y);
        this.img = image;
        this.direction = direction;
    }

    public FlameMulti(int x, int y, Image image){
        super(x, y);
        this.img = image;
        this.radius = 2;
    }

    public FlameMulti(int x, int y){
        super(x,y);
    }
    ...
    public void update(){
        if(time < 20){
            ...
            BombermanMultiPlayer.flameList.remove(this);
    ...
    private void create_explosion(){
        BombermanMultiPlayer.flameList.add(new FlameMulti(x, y, Sprite.bomb_exploded.getFxImage(),0));
        //BombermanGame.explosionList.add(new explosion(x, y, Sprite.bo.getFxImage(),4));
        for(int i = 0; i < right; i++){
            FlameMulti e = new FlameMulti(x + size*(i + 1), y);
            if(i == right - 1) {
            ...
            BombermanMultiPlayer.flameList.add(e);
        }
        ...
        for(int i = 0; i < top; i++){
            FlameMulti e = new FlameMulti(x , y - size*(i + 1));
            if(i == top - 1) {
    ...
    private static boolean isCollisionsWall(Rectangle r){
        for(Entity e : BombermanMultiPlayer.stillObjects){
